                    ...
                    <div class="white_box_50px box_shadow_white">
                        ...
                        <div class="row">
                            <div class="primary_input col-md-6">
                                ...
                                {{ Form::select('client_id', $data['clients'], null, ['required' => '', 'id'=>'client_id', 'class' => 'primary_select', 'data-placeholder' => __('case.Select Client'), 'data-parsley-errors-container' => '#client_id_error']) }}
                                        ...
                                        <div class="form-check">
                                            <label class="primary_checkbox d-flex mr-12">
                                                <input type="radio" class="complete_order0 PetRes" id="complete_order0"
                                                    name="type" value="petitioner" {{ $model->client_type == 'petitioner' ? 'checked':'' }}>
                                                <span class="checkmark mr-2"></span> {{ __('client.Petitioner') }}
                                            </label>
                                        </div>
                                    </div>
                                    <div class="primary_input col-md-6">
                                        <div class="form-check">
                                            <label class="primary_checkbox d-flex mr-12">
                                                <input type="radio" class="complete_order0 PetRes" id="complete_order0"
                                                    name="type" value="respondent" {{ $model->client_type == 'respondent' ? 'checked':'' }}>
                                                <span class="checkmark mr-2"></span> {{ __('client.Respondent') }}
                                            </label>
                                        </div>
                                    </div>
                                </div>
                        ...
                        @php
                            $label = $model->client_type =='petitioner' ? __('case.Respondent Name'):__('case.Petitioner Name');
                        @endphp
                        @foreach ($model->caseParticipants as $item)                           
                            <div class="row">
                                <div class="primary_input col-md-5">
                                    <label for="" class="p_r_name">{{ $label }}</label>
                                    {{ Form::text('p_r_name[]', $item->name, ['class' => 'primary_input_field', 'placeholder' => __('case.Name')]) }}
                                </div>
                                <div class="primary_input col-md-5">
                                    <label for="" class="p_r_advocate">{{ $label }}</label>
                                    {{ Form::text('p_r_advocate[]', $item->advocate, ['class' => 'primary_input_field', 'placeholder' => __('case.Name')]) }}
                                </div>
                        ...
                        <div class="row">
                            <div class="primary_input col-md-12">
                                ...
                                {{ Form::select('staff_ids[]', $data['staffs'], $data['selected_staffs'], ['class' => 'primary_select', 'data-placeholder' => __('case.Select Staff'), 'data-parsley-errors-container' => '#staff_id_error', 'multiple', 'id' => 'staff_id']) }}
        ...
        $(document).ready(function () {
            ...
            $(document).on('click', '.PetRes', function() {
                if ($(this).val() == 'petitioner') {
                    $('.p_r_name').html('Respondent Name');
                    $('.p_r_advocate').html('Respondent Advocate');
                } else {
                    $('.p_r_name').html('Petitioner Name');
                    $('.p_r_advocate').html('Petitioner Advocate');
                }
                $('#type').val($(this).val());
            })
            ...
            function changeLabel(type){
                if (type == 'petitioner') {
                    var name = 'Respondent Name';
                    var advocate = 'Respondent Advocate';
                } else {
                    var name = 'Petitioner Name';
                    var advocate = 'Petitioner Advocate';
                }
