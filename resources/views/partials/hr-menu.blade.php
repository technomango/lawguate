...
@php
    ...
    $nav = array_merge($staffs, $roles, $events, $payroll, $pedningStaff,['attendances.index', 'attendance_report.index', 'payroll_reports.index', 'payroll_reports.search', 'attendance_report.search']);
@endphp
...
<li class="{{ spn_nav_item_open($nav, 'mm-active') }}">
        ...
        <ul>
            @if (moduleStatusCheck('Registration') && permissionCheck('registration.lawyer.pending'))
                <li>
                    <a href="{{ route('registration.lawyer.pending') }}" class="{{ spn_active_link($pedningStaff, 'active') }}">
