...
class GeneralSettingsController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            if ($request->ajax()) {
                ...
                Toastr::success(__('setting.General Setting Credentials has been updated Successfully'));
                session()->put('g_set', '1');
    ...
    public function sendMailTest($to, $subject, $body)
    {
        ...
        if (config('configs.mail_protocol')  == "smtp") {

                    Mail::to($to)->send(new TestSmtpMail($attribute));

                return true;

        }elseif (config('configs.mail_protocol')  == "sendmail") {

            $message = (string) view("emails.mail",$attribute);

            $headers = "From:>".env('SENDER_MAIL')." \r\n";
            $headers .= "Reply-To:". config('configs.email') ." \r\n";
    ...
    {
        GeneralSetting::first()->update(['case_layout'=>$request->layout]);
        Toastr::success(__('setting.Layout Chance Successfully'));
    }
}
